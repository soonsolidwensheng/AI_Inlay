...
def get_proportional(kd, center):
    co, index, dist = kd.find(center)
    return dist
def inlayPost(surfaceObj, inlayObj ):
    ...
    for i,f_id in enumerate(_need_move_faces):
        ...
        if (_need_center_faces[i] - f_center).length < 0.1:
            ...
            bpy.ops.transform.translate(value=(0,0, _need_dist_faces[i]), orient_type='NORMAL',  orient_matrix_type='NORMAL', constraint_axis=(False, False, True), mirror=False, use_proportional_edit=True, proportional_edit_falloff='SMOOTH', proportional_size=1.0, use_proportional_connected=True, use_proportional_projected=False, release_confirm=True)
    ...
    for j in range(0,10):
        ...
        if select_face != -1:
            ...
            bpy.ops.transform.translate(value=(0,0,0.1), orient_type='NORMAL',  orient_matrix_type='NORMAL', constraint_axis=(False, False, True), mirror=True, use_proportional_edit=True, proportional_edit_falloff='SMOOTH', proportional_size=last_dist, use_proportional_connected=True, use_proportional_projected=False, release_confirm=True)
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    if bpy.context.mode == 'EDIT_MESH':
        bpy.ops.object.editmode_toggle()
    # out_verts = np.array([x.co for x in inlay_bm.verts])
    out_verts = np.array([ v.co for v in inlayObj.data.vertices ])
    out_faces = np.array([tuple(polygon.vertices) for polygon in inlayObj.data.polygons[:]], dtype="object")  
...
def inlayPostWarp(surface_verts, surface_faces,inlay_verts, inlay_faces ):
    ...
    convertMesh( 'inlay', inlay_verts, inlay_faces )
    inlayObj = bpy.context.active_object
    out_verts, out_faces = inlayPost(surfaceObj, inlayObj)  
    if bpy.context.mode == 'EDIT_MESH':
        bpy.ops.object.editmode_toggle()
    return out_verts, out_faces
if __name__ == '__main__':
    ...
    inlay_outer = trimesh.load(f'{f_name}/12_final_inlay_outer_46.ply')
    stitched_inlay = trimesh.load(f'{f_name}/13_stitched_inlay_46.ply')
    ...
    out_verts, out_faces = inlayPostWarp(surface_verts, surface_faces, inlay_verts, inlay_faces )
    
    trimesh.Trimesh(out_verts, out_faces).export('out.stl')
    ...
    # surfaceObj = bpy.data.objects['12 Final Inlay Outer 27']
    # inlayObj = bpy.data.objects['13 Stitched Inlay 27']
    # inlayPost(surfaceObj, inlayObj)  
