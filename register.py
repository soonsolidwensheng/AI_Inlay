...
class MeshRegistration:
    ...
    def poisson_reconstruct(self, mesh, outer):
        ...
        if len(outer.faces) > 20000:
            outer = outer.simplify_quadric_decimation(face_count=20000)
        if len(np.array(mesh.triangles)) > 10000:
            mesh = trimesh.Trimesh(mesh.vertices, mesh.triangles)
            mesh = mesh.simplify_quadric_decimation(face_count=10000).as_open3d
            mesh.compute_vertex_normals()
