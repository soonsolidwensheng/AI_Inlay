...
class MeshRegistration: 
    ...
    def reverse_trimesh(self, mesh):
        ...
        reversed_faces = np.column_stack([temp_faces[:, 1], temp_faces[:, 0], temp_faces[:, 2]])  
    ...
    def doTPS(self, mesh, pcd, last_index, last_points, alph):
        ...
        '''
        last_point_dst = alph*(last_points[:,1] - last_points[:,0]) + last_points[:,0]
        trans = tps.TPS(np.asarray(pcd.points)[last_index], last_point_dst, self.tpsAlpha)
    ...
    def getNoLine(self, target, source, normalSize = 2.0):
        search_param = o3d.geometry.KDTreeSearchParamHybrid(radius=normalSize, max_nn=int(30*normalSize))  
        ...
        for i in key_map:
            ...
            if len(sorted_items)>0:
                ...
                if self.draw:points.append(tempPoints)
        if self.draw: self. draw_tps_result(points, [target, source])
    ...
    def register_icpP2F(self, source, target):
        ...
        target.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.5*threshold, max_nn=100))
        reg_p2l = o3d.pipelines.registration.registration_icp(
            source, target, threshold, trans_init,
            o3d.pipelines.registration.TransformationEstimationPointToPlane(),
            o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=1000))
        if self.draw: self.draw_registration_result(source, target, reg_p2l.transformation)
        return reg_p2l

    def poisson_reconstruct(self, mesh):
        """
        ...
        bound_pc = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(np.array(bound_pts)))
        ...
        pcd_q = pcd_q.random_down_sample(sampling_ratio =0.1) # down sample the point cloud

        pcd_q += bound_pc # but keep all the boundary points
        pcd_q = pcd_q.remove_duplicated_points()

        poisson_mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(pcd_q, depth=7, scale=1.5, linear_fit=False, n_threads=multiprocessing.cpu_count())[0]
    ...
    def calcuBsubQ(self, mesh_b, mesh_q):
        ...
        for i in range(len(pcd_b.points)):
            if not i in boundary_q_index:
                pcd_b_q.points.append(pcd_b.points[i])

        #calcu boundary point index
        boundary_q_index = list(set(np.array(mesh_q.get_non_manifold_edges(allow_boundary_edges=False)).flatten()))
        boundarySet = []
    ...
    def liftingMesh(self, mesh, liftDis, initDis, redius):
        boundary_index = list(set(np.array(mesh.get_non_manifold_edges(allow_boundary_edges=False)).flatten()))
        ...
        redius_Distance_array = np.where(redius_Distance_array < redius, redius_Distance_array/redius, 1)
        liftDis = (redius_Distance_array*(liftDis - initDis) + initDis).reshape(redius_Distance_array.shape[0], 1)
    ...
    def get_cement_gap(self, mesh_q):
        ...
        for k in range(len(mesh_q.vertices)):
            ...
            cos_angles = [np.dot(a, x) for x in b]
            angle = np.min(np.arccos(cos_angles) / 3.141592653 * 180)
            if angle > 60:
                n.append(k)
                points.append(mesh_q.vertices[k])
                continue
    ...
    def doLastMeshInsertTps(self, mesh_source, boundarySet):
        ...
        for face_idx in faces:  
            flag = True
            for i in face_idx:
                if i in lastIndexSet: flag = False
            if flag:
                lastFaces.append(face_idx)

        lastMesh = trimesh.Trimesh(vertices=mesh_source.vertices, faces=np.asarray(lastFaces))
        if self.draw: lastMesh.show()
    ...
    def fstBoudaryTpsVec(self, mesh_source, boundarySet, alph):
        ...
        if len(lastIndexMap) < int(0.25*len(boundarySet)): return (True, mesh_source)
        lastIndex, lastPoint = [], []
        points = []
        for i in lastIndexMap:
            lastIndex.append(i)
            lastPoint.append([pcd.points[i], pcd.points[i] + lastIndexMap[i]])
            if self.draw: points.append([pcd.points[i], pcd.points[i] + lastIndexMap[i]])
        if self.draw: self.draw_tps_result(points, [pcd])
    ...
    def doBoundaryTps(self, mesh_source, boundarySet, alph = 0.3, iterNum = 10):
        ...
        for i in range(iterNum):
            mult = mult_lev_min*mult + (1-mult_lev_min)*mult*(iterNum-1 - i)/(iterNum - 1)
            alph = alph + (1-alph)*(i)/(iterNum - 1)
            flag, mesh_tps = self.lastBoudaryTpsVec(mesh_source, boundarySet, alph, mult)
    ...
    def getPcdKeyPoints(self, pcd, salient_radius=0.0,
                                   non_max_radius=0.0,
                                   gamma_21=0.975,
                                   gamma_32=0.975,
                                   min_neighbors=5):
    ...
    def subMeshBoundaryTps(self, dstboundary, sub_mesh, iterNum = 2):
        ...
        outer_bound_pid = list(set(np.array(sub_mesh.get_non_manifold_edges(allow_boundary_edges=False)).flatten()))
        ...
        while iterNum > 0:
            ...
            for i in boundary_index: # add boundary keypoints to move the margin
                ...
                [k, idx, _] = bound_tree.search_knn_vector_3d(p, 1) # find the closest point on the margin of inner (Q)
                l = len(dstboundary)
                d = -1
                p0 = p
                ...
                index.append(i)
                points.append([p, pl])
            tempSet = set(boundary_index) # add boundary verts to the index list for stretching the margin

            # To keep the non-boundary points stay in the same place is to use all verts far from the outer margin
            for i, p in enumerate(np.asarray(sub_pc.points)): # keep the non-boundary points stay in the same place
                if i in tempSet:
                    continue
                pcd_source = o3d.geometry.PointCloud(o3d.utility.Vector3dVector([p]))
                if pcd_source.compute_point_cloud_distance(outer_bound_pc)[0] < self.boundaryTPS_range:
                    continue
                index.append(i)
                points.append([p, p])

            if self.draw: self.draw_tps_result(points, [pcd, pcd_bound])
            ...
            if self.isSave:
                ...
                o3d.io.write_point_cloud(os.path.join(self.save_path, f'6.1_bound_tps_src_{self.teeth_num}.ply'), pts1)
                o3d.io.write_point_cloud(os.path.join(self.save_path, f'6.2_bound_tps_dst_{self.teeth_num}.ply'), pts2)
    ...
    def doSubMeshTps(self, dstboundary, sub_mesh, liftDis = 0.5):
        ...
        # keypoints = nextPcd.farthest_point_down_sample(100)
        ...
        dis_to_boundary = np.asarray(keypoints.compute_point_cloud_distance(boundary_pc))
        keypoints = keypoints.select_by_index(np.where(dis_to_boundary > 1.0)[0])
        ...
        points = np.asarray(points)
        tps_mesh_ = self.doTPS(tps_mesh, nextPcd, index, points, self.outer_pre_lift_dis) # lift the outer for 1 mm 
        
        mesh = self.subMeshBoundaryTps(dstboundary, self.tri2o3d(tps_mesh_)) #strech the outer to meet Q margin
    ...
    def boundaryDel(self, boundaryPoints, mesh):
        ...
        for face_idx in faces:  
            ...
            if flag:
                lastFaces.append(face_idx)
        lastMesh = trimesh.Trimesh(vertices=np.asarray(mesh.vertices), faces=np.asarray(lastFaces))
    ...
    def remove_small_connected_components(self, mesh):
        ...
        triangles_to_remove = np.where(triangle_clusters != largest_cluster_idx)[0].astype(np.int32)
        noise_removed.remove_triangles_by_index(triangles_to_remove)
        noise_removed.remove_unreferenced_vertices()
        return noise_removed

    def ensure_thickness(self, inlay_outer, inlay_inner, 
                         thickness=0.8,
        ...
        # np.savetxt('verts_outer.txt',verts_outer, fmt='%.16f')
        # np.savetxt('faces_outer.txt', faces_outer, fmt='%i')
        # np.savetxt('verts_inner.txt', verts_inner, fmt='%.16f')
        # np.savetxt('faces_inner.txt', faces_inner, fmt='%i')
        # temp = np.array(groove_vert_ids)
        # np.savetxt('groove_vert_ids.txt', temp, fmt='%i')

        verts, faces = inlayAdjust(verts_outer, faces_outer, verts_inner, faces_inner, groove_vert_ids,
                                   thickness=thickness,
    ...
    def run(self, mesh_source, mesh_target, mesh_q):
        ...
        if self.isSave:
            o3d.io.write_triangle_mesh(os.path.join(self.save_path, f'0_lib_tooth_{self.teeth_num}.stl'), mesh_source)
            o3d.io.write_point_cloud(os.path.join(self.save_path, f'1_b_sub_q_{self.teeth_num}.ply'), pcd_b_q)
            mesh.export(os.path.join(self.save_path, f'2_registreation_{self.teeth_num}.stl'))
        ...
        if self.isSave:
            self.inner_dilation.export(os.path.join(self.save_path, f'3_dilation_0.04-0.08_{self.teeth_num}.stl'))
        ...
        if self.isSave:
            has_boundary_mesh.export((os.path.join(self.save_path, f'4_boundary_TPSed_{self.teeth_num}.stl')))
        # thickness=================================================================
        t1 = time.time()
        has_boundary_mesh = self.ensure_thickness(has_boundary_mesh, 
                                                self.inner_dilation, 
                                                thickness=self.thickness, 
                                                boundary_protect_range_offset=self.boundary_protect_range_offset,
                                                lib_tooth_config=self.lib_tooth_configs)
        if self.isSave:
            has_boundary_mesh.export(os.path.join(self.save_path, f'5_inflated_outer_{self.teeth_num}.stl'))
        ...
        if self.isSave:
            o3d.io.write_triangle_mesh(os.path.join(self.save_path, f'7_poisson_mesh_{self.teeth_num}.stl'), poisson_mesh)        
        t1 = time.time()
        ...
        if self.isSave:
            ...
            o3d.io.write_triangle_mesh(os.path.join(self.save_path, f'8_submesh_{self.teeth_num}.stl'), sub_mesh)
        ...
        if self.isSave:
            sub_mesh.export(os.path.join(self.save_path, f'9_tps_submesh_{self.teeth_num}.stl'))
        #unit surface=====================================================================
...
def main():
    ...
    for _, dirs, __ in os.walk(path):  
        for dir in dirs:  
            print(dir)
            if dir != "cb1b-6637": continue
            if not (dir in nameDic): continue  
            sub_path = dir
