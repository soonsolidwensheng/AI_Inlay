...
def subdivide_loop(vertices,
    ...
    def _subdivide(vertices, faces):
        ...
        if edge_bound_mask.any():
            ...
            p_id = np.where(np.sum(boundary_neighbors>0, axis=1) > 2)[0]
            p_vrt_id = np.where(vrt_bound_mask)[0][p_id]
            edge_id = np.where(np.isin(edges[unique][~edge_inter_mask], p_vrt_id))[0]
            error_edge = edges[unique][~edge_inter_mask][edge_id].reshape(-1, 2, 2)
            for e_i in range(len(error_edge)):
              boundary_neighbors[p_id[e_i]][~np.isin(boundary_neighbors[p_id[e_i]], error_edge[e_i])] = -1
